...
### Быстрая сборка
```bash
# Полная сборка с PWA, цветами и перезапуском
cd /root/florist-app
./build-and-deploy.sh
```

### Ручная сборка компонентов
```bash
# Только frontend
...
# Только исправление цветов
cd /root/florist-app
./fix-colors.sh
```

### Управление кэшем
- **Автоматическое обновление** - кэш PWA обновляется при каждой сборке
- **Принудительное обновление** - выполняется при запуске `build-and-deploy.sh`
- **Версионирование** - Service Worker автоматически получает новую версию
...
│   ├── public/                  # Статические файлы (frontend + PWA)
│   │   ├── assets/             # JS/CSS файлы frontend
...
│   │   │   ├── PWAInstallPrompt.vue # Кнопка установки PWA
│   │   │   ├── LoginForm.vue   # Форма входа
│   │   │   └── MobileNav.vue   # Мобильная навигация
│   │   ├── pages/              # Страницы приложения
│   │   └── utils/              # Утилиты
│   ├── dist/                    # Собранный frontend (копируется в backend/public)
│   ├── public/                  # Статические файлы для сборки
│   │   ├── manifest.webmanifest # PWA манифест (исходник)
│   │   └── pwa-*.png           # PWA иконки (исходники)
│   ├── package.json             # Зависимости frontend
│   └── vite.config.ts           # Конфигурация Vite + PWA плагин
├── scripts/
│   └── clean-logs.sh            # Очистка логов
├── build-and-deploy.sh          # Комплексная сборка и развертывание
├── fix-colors.sh                # Исправление цветов в собранных файлах
...
- **Автоматическая установка** - кнопка установки появляется только когда браузер поддерживает PWA
- **Работа оффлайн** - приложение работает без интернета благодаря Service Worker
- **Нативный вид** - выглядит как обычное мобильное приложение после установки
- **Push-уведомления** - поддержка уведомлений (требует настройки)
- **Автоматическое обновление** - кэш обновляется при изменении файлов
- **Кроссплатформенность** - работает на iOS, Android, Desktop

**Поддерживаемые браузеры для установки:**
- Chrome (Android/Desktop) - полная поддержка
- Safari (iOS) - через "Поделиться" → "На экран Домой"
...
### 🎨 Система тематизации
- **Единый цвет темы**: #E63A62 (розовый) применяется ко всему приложению
- **Автоматическая коррекция цветов** - скрипт `fix-colors.sh` заменяет нежелательные цвета
- **Поддержка темной темы** - автоматическое переключение по системным настройкам
- **PWA иконки** в фирменных цветах

**Файлы тематизации:**
- `fix-colors.sh` - скрипт замены цветов в собранных файлах
- `build-and-deploy.sh` - комплексная сборка с применением темы
...
- **PWA безопасность** - HTTPS required для полной функциональности PWA
- **Service Worker** - кэширование только безопасного контента

## 🧹 Очистка и обслуживание

### Очистка логов
```bash
# Автоматическая очистка всех логов
./scripts/clean-logs.sh

# Ручная очистка конкретных логов
> logs/webhooks/all_webhooks.log
...
# Очистка node_modules (при проблемах с зависимостями)
...
sqlite3 backend/florist.sqlite "DELETE FROM order_ids WHERE order_id LIKE 'TEST%' OR order_id LIKE 'FINAL%';"

# Проверка размера файлов
du -sh backend/public/assets/*
du -sh logs/*
```
